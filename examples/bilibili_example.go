...
func main() {
	...
	if videoResp != nil && videoResp.Code == 0 {
		commentsResp, err := bilibili.GetCommentsFallback(videoResp.Data.AID, 1, 20)
		if err != nil {
	...
	// 示例：获取评论信息（使用Cookie认证）
	//fmt.Println("获取评论信息示例（使用Cookie认证）:")
	// 注意：这里需要替换为实际的SESSDATA
	// sessdata := "your_sessdata_here"
	// if videoResp != nil && videoResp.Code == 0 {
	// 	commentsResp, err := bilibili.GetComments(videoResp.Data.AID, 1, 20, bilibili.WithCookie(sessdata))
	// 	if err != nil {
	// 		log.Printf("获取评论失败: %v", err)
	// 	} else if commentsResp.Code != 0 {
	// 		log.Printf("获取评论失败，错误码: %d, 错误信息: %s", commentsResp.Code, commentsResp.Message)
